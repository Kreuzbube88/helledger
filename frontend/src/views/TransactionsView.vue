...
const filterAutoGenerated = ref('__all__')
...
function flattenCats(list) {
  ...
  function walk(items, depth) {
    for (const c of items) {
      result.push({ ...c, _isChild: depth > 0 })
...
async function load() {
  ...
  if (res.ok) {
    ...
    if (filterAutoGenerated.value === 'auto') {
      transactions.value = all.filter(tx => tx.is_auto_generated === true)
    } else if (filterAutoGenerated.value === 'manual') {
      transactions.value = all.filter(tx => tx.is_auto_generated !== true)
    } else {
  ...
  <div class="min-h-dvh bg-background">
    <main class="max-w-5xl mx-auto px-4 py-6">
      ...
      <div class="flex gap-3 mb-4 flex-wrap">
        ...
        <Select v-model="filterAutoGenerated" @update:modelValue="load">
          <SelectTrigger class="w-44">
            <SelectValue :placeholder="t('transactions.filterAuto')" />
          </SelectTrigger>
          <SelectContent>
            <SelectItem value="__all__">{{ t('transactions.filterAll') }}</SelectItem>
            <SelectItem value="manual">{{ t('transactions.filterManual') }}</SelectItem>
            <SelectItem value="auto">{{ t('transactions.filterAutoOnly') }}</SelectItem>
          </SelectContent>
        </Select>
      </div>
            ...
            <TableRow v-for="tx in transactions" :key="tx.id">
              ...
              <TableCell class="max-w-48 truncate">
                <span class="inline-flex items-center gap-1.5">
                  ...
                  <Badge v-if="tx.is_auto_generated" variant="outline" class="text-[10px] px-1 py-0 h-4">{{ t('transactions.autoLabel') }}</Badge>
                </span>
              </TableCell>
